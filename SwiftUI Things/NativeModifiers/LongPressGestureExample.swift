import SwiftUI
import iOS26Macros

extension LongPressGestureExample: NativeModifiersThing {
    static let title = "LongPressGesture"
    static func makeView() -> some View { Self() }
}

struct LongPressGestureExample: View {

    @GestureState private var press = false
    @State private var show = false

    var body: some View {
        let (preview, code) = #CodeSnippet(
            Image(systemName: "camera.fill")
                .foregroundColor(.white)
                .frame(width: 60, height: 60)
                .background(show ? Color.black : Color.blue)
                .mask(Circle())
                .scaleEffect(press ? 2 : 1)
                .animation(.spring(response: 0.4, dampingFraction: 0.6))
                .gesture(
                    LongPressGesture(minimumDuration: 0.5)
                        .updating($press) { currentState, gestureState, transaction in
                            gestureState = currentState
                        }
                        .onEnded { value in
                            show.toggle()
                        }
                )
        )

        Storybook(
            title: Self.title,
            badges: [
                .init(title: "Native Modifier", icon: "wand.and.stars"),
            ],
            description: "A camera icon that scales to 2x on press using LongPressGesture with .updating to drive a spring animation. Toggle the background color on long press completion.",
            code: code,
            preview: { preview }
        )
    }
}

#Preview {
    LongPressGestureExample()
}
